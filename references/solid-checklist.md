...
- Barrel files re-exporting entire modules (coupling)
- Circular dependencies between modules

### Go
- Package-level `init()` functions with side effects
- Returning concrete types instead of interfaces
- Giant `main()` function doing everything

### Python
- God class with `__init__` accepting 10+ parameters
- Mixing sync and async code without clear boundaries
- Module-level mutable state

### Java
- Service classes with 15+ injected dependencies
- Utility classes with only static methods (procedural disguised as OOP)
