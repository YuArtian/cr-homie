...
- **Error type confusion**: Throwing strings instead of Error objects, losing stack trace
...
### Logging
- Critical operations missing log entries (auth events, data mutations, payment flows)
- Insufficient context in log messages (missing request ID, user ID, operation)
- Excessive DEBUG logging in production code paths
- Sensitive data logged (passwords, tokens, PII)

### Metrics & Monitoring
- New failure modes without corresponding alerts
- Missing latency/throughput metrics for new endpoints
- No health check for new external dependencies
- Error rates not tracked for new code paths

### Questions to Ask
- "If this fails in production, will we know? How quickly?"
- "Is there enough information in logs to debug without reproducing?"
- "What dashboard would show a regression from this change?"
